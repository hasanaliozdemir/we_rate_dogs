...
class DogAddPage extends StatelessWidget {
  final String time;
  const DogAddPage({Key key, this.time}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      title: 'We Rate Dogs - Signup Dog',
      theme: ThemeData.dark(),
      home: MyHomePage(title: 'Signup Form',time: time,),
    );
  }
}
...
class MyHomePage extends StatefulWidget {
  ...
  const MyHomePage({Key key, this.title,this.time}) : super(key: key);
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  int genderConvertNum;
  final GlobalKey<FormState> _formKey = GlobalKey<FormState>();

  final _formResult = Dog.withoutInfo();
          ...
          child: ListView(
            ...
            children: <Widget>[
              ...
              MyToggleButtonsFormField<Gender>(
                ...
                onSaved: (gender) {
                  if (gender == Gender.Male) {
                    _formResult.genderNum = 1;
                  }
                  if (gender == Gender.Female) {
                    _formResult.genderNum = 2;
                  }
                  if (gender == Gender.Other) {
                    _formResult.genderNum = 0;
                  }
                },
              ...
              MyDateFormField(
                ...
                onSaved: (birthdate) {
                  _formResult.birthdate = birthdate.toString();
                },
  ...
  Future<void> _submitForm() async {
    final imageUrl = await urlGetter();
    ...
    if (form.validate()) {
      ...
      print(_formResult);
      firestoreInstance.collection("dogs").add({
        "name": _formResult.name,
        "birthdate": _formResult.birthdate,
        "genderNum": _formResult.genderNum,
        "ethicsAgreement": _formResult.ethicsAgreement,
        "vote": 0,
        "image_url": imageUrl,
      }).then((value) {
